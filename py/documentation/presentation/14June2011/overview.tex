...
\begin{frame}
  ...
  \only<2->{In a nutshell:}

  \begin{itemize}
  \item<1-> This is the subject of Todd and Pat's discussion which
    will follow.
  \item<2-> We need flexibile implementation.
  \item<3-> We need flexible configurations and data management.
  \end{itemize}
...
\begin{frame}
  ...
  \only<1>{\includegraphics[height=5.0cm]{agent}}
  \only<2>{\includegraphics[height=5.0cm]{agentRouter}}
  \only<3>{\includegraphics[height=5.0cm]{agentSocketRouter}}
...
\begin{frame}
  ...
  \item Data exchange between agents within simulation should be
    consistent with outside requests.
